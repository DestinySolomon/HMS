    ...
    <DashboardLayout>
      <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-400 sm:text-base">
                Monitor occupancy, revenue, guest service, and finance in a refined dashboard built for a modern luxury property.
              </p>
            </div>
            <div className="rounded-2xl border border-[#243041] bg-[#0B1220] px-4 py-3 text-sm text-slate-300">
              ...
              <p className="mt-1 text-slate-400">Guest satisfaction and room readiness</p>
            </div>
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <Card icon={<DollarSign />} title="Revenue Today" value="₦4.8M" />
          <Card icon={<BedDouble />} title="Occupied Rooms" value="62" />
          <Card icon={<Users />} title="Active Guests" value="145" />
          <Card icon={<TrendingUp />} title="Monthly Growth" value="+18%" />
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
          <div className="rounded-3xl border border-[#243041] bg-[#101826] p-6 shadow-xl shadow-black/20">
            <div className="flex items-center justify-between">
              ...
              <span className="rounded-full border border-[#C8A45D]/30 bg-[#C8A45D]/10 px-3 py-1 text-sm text-[#F5D88A]">+12% vs last month</span>
            </div>
            <div className="mt-6 h-72">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={revenueData}>
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey="revenue" stroke="#C8A45D" strokeWidth={3} dot={{ r: 4, fill: "#F5D88A" }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-[#243041] bg-[#101826] p-6 shadow-xl shadow-black/20">
            ...
            <div className="mt-6 h-72">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={roomData} dataKey="value" innerRadius={55} outerRadius={90} paddingAngle={3}>
            ...
            <div className="space-y-2 text-sm text-slate-400">
              {roomData.map((item) => (
                <div key={item.name} className="flex items-center justify-between rounded-xl bg-[#0B1220] px-3 py-2">
                  <span>{item.name}</span>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr_0.95fr]">
          <div className="rounded-3xl border border-[#243041] bg-[#101826] p-6 shadow-xl shadow-black/20">
            ...
            <div className="mt-5 space-y-3">
              {activities.map((activity, index) => (
                <div key={index} className="rounded-2xl border border-[#243041] bg-[#0B1220] px-4 py-3 text-sm text-slate-300">
                  {activity}
                </div>
              ))}
            </div>
          ...
          <div className="rounded-3xl border border-[#243041] bg-[#101826] p-6 shadow-xl shadow-black/20">
            <h3 className="text-lg font-semibold text-white">Financial snapshot</h3>
...
function Card({ icon, title, value }) {
  return (
    <div className="rounded-3xl border border-[#243041] bg-gradient-to-br from-[#101826] to-[#162132] p-6 shadow-lg shadow-black/20">
      <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#C8A45D]/15 text-[#F5D88A]">
        {icon}
      </div>
